...
def updateQTable(q_table, state, reward, action, new_state):
    ...
    if newPosition >= 0.5:
        ...
        bestAction, _ = getMaxFutureValue(q_table, newPosition, newVelocity)
        newQValue = currentQValue + learningRate*(reward + discountFactor*bestAction - currentQValue)
...
def runEpisode(q_table):
    ...
    running = True
    while running:
        # chose the best future action
        _, action = getMaxFutureValue(q_table, state[0], state[1])
