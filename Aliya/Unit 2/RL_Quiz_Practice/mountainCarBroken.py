...
def discretizePosition(position):
    index = position + 1.2
    index = index // 0.1 # (1) floor division by 0.1 not 0.01 to get 18 indices rather than 180
    return int(index)

def discretizeVelocity(velocity):
    index = velocity + 0.07
    index = index // 0.01 # (2) floor division by 0.01 not 0.001 to get 14 indices rather than 140
    return int(index)

def getMaxFutureValue(q_table, position, velocity):
    positionIndex = discretizePosition(position) # (3) need to use the discretize position function to get the index
    velocityIndex = discretizeVelocity(velocity) # (4) need to use the discretize velocity function to get the index

    bestAction = -100
    actionNumber = random.choice(ACTIONS)
    for action in ACTIONS:
        if q_table[action][positionIndex][velocityIndex] > bestAction: # (5) replace bestAction with the q value at that place if it is greater not less than
...
def updateQTable(q_table, state, action, reward, new_state, episode):
    ...
    if new_state[0] >= 0.5:
        newQ = 100 # (6) if we reach the goal, we should give a positive reward, not negative
        ...
        newQ = currentQ + 0.4*(maxFutureVal + 0.7*reward - currentQ) 
