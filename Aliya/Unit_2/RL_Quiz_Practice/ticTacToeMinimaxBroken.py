...
def recursiveMinimax(board, player):
    ...
    if check_win(board, "X"):
        return 5, bestMove # (1) should be a negative reward because don't want the opponent X to win
    if check_win(board, "O"):
        return -5, bestMove # (2) should be positive 5 reward because want to reward O, the minimax bot, when it wins
    if is_board_full(board):
        return 0, bestMove # (3) should be 0 reward because do not want to reward or penalize if tie
    
    # setup recursive case
    # check all possible spots and calculate the max/min score recurisively
    for i in range(len(board)):
        for j in range(len(board)): # (4) range should not be len(board)-1 because last column (col 2) is excluded
            if board[i][j] == " ":
                #place move temporarily
                board[i][j] = player
                score, _ = recursiveMinimax(board, opponent) # (5) should see the score based on what the opponent can do, if our player move lets the opponent win
...
def tic_tac_toe():
    ...
    while True:
        print_board(board)
        if current_player == 'X':
            minimax_bot_move(board, 'X')
        else:
            random_bot_move(board, 'O')
